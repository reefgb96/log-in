...
const modalTriggerBtn = document.getElementById("button");
...
document.onkeydown = (event) => {
  if (event.keyCode === 27) modalBg.style.display = "none";
};

let userName = document.getElementById("user-name");
let userPass = document.getElementById("pass");
let logInSubmit = document.querySelector(".submit-button");
...
const logInFunc = () => {
  ...
  user.innerHTML = `Welcome back, ${localStorage.getItem('UserName')}`;
};

const logOutFunc = () => {
  modalTriggerBtn.style.display = "flex";
...
logInSubmit.addEventListener("click", () => logInFunc());
